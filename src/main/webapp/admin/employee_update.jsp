            ...
            <div class="row">
                ...
                <form action="admincrud" method="POST">
                    <div class="form-group row">
                        員工姓名
                        <div class="col-sm-10">
                            <input class="form-control form-control-sm" type="text" name="admin_name" value="<%=request.getParameter("admin_name")%>" required="required">
                        </div>
                    </div>                   
                    <div class="form-group row">
                        員工帳號
                        <div class="col-sm-10">
                            <input class="form-control form-control-sm" type="text" onkeyup="this.value=this.value.replace(/\s+/g,'')" name="admin_account" value="<%=request.getParameter("admin_account")%>" required="required" pattern="[a-z0-9A-Z]{0,20}" title="必填，請輸入20位以下的英文與數字">
                        </div>
                    </div>  
                    <div class="form-group row">
                        員工密碼
                        <div class="col-sm-10">
                            <input class="form-control form-control-sm" type="password" onkeyup="this.value=this.value.replace(/\s+/g,'')" name="admin_password" value="<%=request.getParameter("admin_password")%>" required="required">
                        </div>
                    </div> 
                    <div class="form-group row">
                        員工電話
                        <div class="col-sm-10">
                            <input class="form-control form-control-sm" type="tel" onkeyup="this.value=this.value.replace(/\s+/g,'')" name="admin_phone" value="<%=request.getParameter("admin_phone")%>" required="required" pattern="[0-9]{0,}" title="必填，請輸入數字">
                        </div>
                    </div>
                    <div class="form-group row">
                        是否啟用
                        <div class="col-sm-10">
                            <input type="radio" name="admin_active" value="1" checked>是　<input type="radio" name="admin_active" value="0">否
                        </div>
                    </div>
                    <div id="our_button">
                        <br>
